...
```swift
.package(url: "https://github.com/superuser404notfound/AetherEngine", from: "3.5.0")
```
...
Pin `from: "3.5.0"` in your `Package.swift` to allow patch + minor updates while excluding breaking changes:

```swift
.package(url: "https://github.com/superuser404notfound/AetherEngine", from: "3.5.0")
```

Pin to `.upToNextMinor(from: "3.5.0")` for stricter teams that prefer to opt into minor bumps explicitly. See [CHANGELOG.md](CHANGELOG.md) for the per-release index.
