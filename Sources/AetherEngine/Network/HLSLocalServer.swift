...
protocol HLSSegmentProvider: AnyObject {
    ...
    /// Decoy subtitle renditions to advertise in the master playlist so AVKit's native subtitle picker lists them. Each entry produces an `#EXT-X-MEDIA:TYPE=SUBTITLES` line in a "subs" group and a `SUBTITLES="subs"` attribute on the variant. The server serves a fixed empty (cue-less) WebVTT rendition for each: the host paints the actual cues itself. Empty (default) means no subtitle lines and no behavioral change. `renditionID` is a path-safe token (the server matches `/subs_<renditionID>.m3u8` and `.vtt`). (Rivulet fork-only.)
    var subtitleRenditions: [(renditionID: String, name: String, language: String)] { get }

    /// Media duration in seconds, used to size the decoy WebVTT rendition's single `#EXTINF`. nil when the provider does not know the duration (live), in which case the server uses a large constant so the empty cue list spans the whole asset. (Rivulet fork-only.)
...
extension HLSSegmentProvider {
    ...
    /// Default: no decoy subtitle renditions (feature off / providers that never carry subtitles). (Rivulet fork-only.)
    var subtitleRenditions: [(renditionID: String, name: String, language: String)] { [] }

    /// Default: duration unknown. (Rivulet fork-only.)
