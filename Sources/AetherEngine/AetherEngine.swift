...
public final class AetherEngine: ObservableObject {
    ...
    /// Maps an advertised decoy subtitle rendition (HLS master playlist, native-picker path) back to the engine subtitle track it stands for. Non-empty only when the active load set `LoadOptions.advertiseSubtitleRenditions == true`; empty otherwise and whenever no session is loaded. The host uses `renditionID` / `trackIndex` to correlate AVKit's selected legible option with the `TrackInfo` whose cues it should then decode + paint. (Rivulet fork-only.)
    ...
    public func load(
        ...
        subtitleRenditions = options.advertiseSubtitleRenditions
            ? Self.makeSubtitleRenditions(from: probedSubtitleTracks,
                                          bitmapOnly: options.prepareNativeSubtitles)
            : []
