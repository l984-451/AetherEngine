...
final class MP4SegmentMuxer {
    ...
    /// NOTE on the BoxRecord (tail bytes [10..18] = top,left,bottom,right as 4×uint16): ffmpeg emits an
    /// all-zero default text box. A 0×0 box makes AVPlayer DECODE the tx3g cues but render them into an
    /// empty region → "plays but no subtitles visible" (sim-confirmed). We override it to the full
    /// 1920×1080 track region (bottom=0x0438=1080, right=0x0780=1920) so AVPlayer has an area to draw in.
    static let movTextSampleDescriptionExtradata: [UInt8] = [
        0x00, 0x00, 0x00, 0x00, 0x01, 0xff, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00,
        0x00, 0x00, 0x04, 0x38, 0x07, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
