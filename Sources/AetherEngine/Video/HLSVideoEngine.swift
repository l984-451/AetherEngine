...
public final class HLSVideoEngine: @unchecked Sendable {
    ...
    /// Decoy subtitle renditions to advertise in the master playlist (native-picker path). Supplied by the engine at construction; forwarded to the `VideoSegmentProvider` when it is built in `start()`. Empty means the feature is off. (Rivulet fork-only.)
    ...
    public func start() throws -> URL {
        ...
        self.provider = prov
        // Hand the decoy subtitle renditions to the provider before the server starts, so the very first
        // master.m3u8 fetch already carries the #EXT-X-MEDIA subtitle lines (native-picker path). Empty when
        // the feature is off; provider then emits no subtitle lines and the master is byte-identical. (Rivulet fork-only.)
        prov.subtitleRenditionList = subtitleRenditions
        // Live producer appends each finalized segment to the provider's growing list so the live playlist exposes it on the next poll.
        if isLiveSession {
