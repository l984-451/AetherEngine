...
final class VideoSegmentProvider: HLSSegmentProvider, @unchecked Sendable {
    ...
    /// Decoy subtitle renditions to advertise in the master playlist (native-picker path). Set by the engine at load time before the server is started; empty means the feature is off and the master emits no subtitle lines. Plain stored value: written once on the load thread before `HLSLocalServer.start()`, then read-only for the session, so it needs no lock. (Rivulet fork-only.)
    var subtitleRenditionList: [(renditionID: String, name: String, language: String)] = []

    /// Total source duration in seconds (0 for live / unknown), used to size the decoy WebVTT rendition playlist's single EXTINF. (Rivulet fork-only.)
